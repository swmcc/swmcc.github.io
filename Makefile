...
local.run:
	@if [ ! -f worker/.dev.vars ]; then \
		...
		npm run dev; \
	elif curl -sf -o /dev/null -m 1 http://localhost:4321/ || curl -sf -o /dev/null -m 1 http://localhost:4322/; then \
		echo "🥩 Site dev server already running — starting Swanson worker in the foreground"; \
